...
// Additional convenience macros for the test program
#define LLAMA_RESOURCE_BEGIN_SESSION(session_id) \
    if (g_resource_instr) g_resource_instr->begin_session(session_id)

#define LLAMA_RESOURCE_END_SESSION() \
    if (g_resource_instr) g_resource_instr->end_session()

#define LLAMA_RESOURCE_BEGIN_LAYER(layer_id) \
    if (g_resource_instr) g_resource_instr->begin_layer(layer_id)

#define LLAMA_RESOURCE_END_LAYER(layer_id) \
    if (g_resource_instr) g_resource_instr->end_layer(layer_id)

#define LLAMA_RESOURCE_BEGIN_COMPONENT(component_type) \
    if (g_resource_instr) g_resource_instr->begin_component(component_type)

#define LLAMA_RESOURCE_END_COMPONENT(component_type) \
    if (g_resource_instr) g_resource_instr->end_component(component_type)

#define LLAMA_RESOURCE_LOG_COMPONENT_HANDOFF(from_component, to_component) \
    if (g_resource_instr) g_resource_instr->log_component_handoff(from_component, to_component)
